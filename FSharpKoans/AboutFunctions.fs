...
module ``03: Putting the Function into Functional Programming`` = 
    ...
    let ``02 The input to a function is a pattern (Part 1).`` () =
        (fun 7 -> 9) 7 |> should equal 9 // basically the input has to match to the identifier 

    [<Test>]
    let ``03 The input to a function is a pattern (Part 2).`` () =
        (fun _ -> 75) 9000 |> should equal 75
    ...
    let ``10 A function can return a function (Part 1).`` () =
        ...
        let j = i 10 // matches with first identifier it finds 
        let k = j 9 // fun 10 -> (fun hate -> (10 - hate)) can't match with 10 so has to match with hate
        k |> should equal 1
    ...
    let ``11 A function can return a function (Part 2).`` () =
        let funky a b = a + b
        let j = funky 10 
        let k = j 9 // funky 10 9 i.e a = 10 and b =9
        k |> should equal 19

    [<Test>]
    let ``12 You can write a function as a one-liner (Part 1).`` () =
        (fun multi -> fun num2 -> multi * num2) 3 9 |> should equal 27
    ...
    *)
        let somefunc x y = x + y x // x is input to function y 
        let square v = v * v 
        somefunc 3 square |> should equal 12 // somefunc 3 square = 3 square 3
        somefunc 3 ((*) 7) |> should equal 24 // (*)7 = y, (*) is th multiplier function somefunc 3 (*)7 = 3 + 3*7
        somefunc 10 ((+) 8) |> should equal 28 // (+)7 = y,function somefunc 10 (+)8 = 10 + 10 +8
